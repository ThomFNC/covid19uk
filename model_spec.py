...
def read_covariates(paths, date_low, date_high):
    ...
    """
    mobility = data.read_traffic_flow(
        paths["mobility_matrix"], date_low=date_low, date_high=date_high
    )
    popsize = data.read_population(paths["population_size"])
    commute_volume = data.read_mobility(paths["commute_volume"])
    ...
    return dict(
        C=commute_volume.to_numpy().astype(DTYPE),
        W=mobility.to_numpy().astype(DTYPE),
