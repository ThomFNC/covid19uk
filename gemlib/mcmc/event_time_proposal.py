...
from gemlib.distributions import Categorical2
...
def EventTimeProposal(
    ...
    def delta_t(t):
        with tf.name_scope("delta_t"):
            ...
            low = -tf.clip_by_value(d_max_bcast, clip_value_min=0, clip_value_max=t)
            high = tf.clip_by_value(
                d_max_bcast, clip_value_min=0, clip_value_max=events.shape[-2] - t - 1
            )
            return UniformInteger(low=low, high=high + 1, float_dtype=events.dtype)
...
def FilteredEventTimeProposal(  # pylint: disable-invalid-name
    ...
    def m():
        with tf.name_scope("m"):
            ...
            X = UniformKCategorical(m_max, hot_meta, float_dtype=events.dtype, name="m")
            return X
    ...
    return tfd.JointDistributionNamed(
        dict(m=m, move=move), name="FilteredEventTimeProposal"
    )
