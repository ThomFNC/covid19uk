...
class AdaptiveRandomWalkMetropolis(kernel_base.TransitionKernel):
    ...
    #### Example 1: 3-variate MVN
    ```python
        import numpy as np
        import tensorflow as tf
        import tensorflow_probability as tfp
        from gemlib.mcmc.adaptive_random_walk_metropolis import AdaptiveRandomWalkMetropolis
        ...
        samples, results = posterior(
            iterations=10000,
            burnin=1000,
            initial_state=dtype([0.1, 0.1, 0.1]))

        tf.print('\nAcceptance probability:',
                 tf.math.reduce_mean(
                    tf.cast(results.is_accepted, dtype=tf.float32)))

        tf.print('sample_mean', tf.math.reduce_mean(samples, axis=0))
        tf.print('sample_cov', tfp.stats.covariance(samples))
    ...
    ```python
        ...
        @tf.function
        def posterior(iterations, burnin, thinning, initial_state, initial_covariance):
            kernel = AdaptiveRandomWalkMetropolis(
        ...
        tf.print('Acceptance probability:',
                tf.math.reduce_mean(
                    tf.cast(results.is_accepted, dtype=tf.float32)))
        tf.print('alpha samples:', samples[:, 0])
        tf.print('beta  samples:', samples[:, 1])
        tf.print('sigma samples:', samples[:, 2])
        tf.print('sample means: [alpha, beta, sigma] =', tf.math.reduce_mean(samples, axis=0))
    ...
    def __init__(
        ...
        with tf.name_scope(
            ...
            if (
                covariance_scaling_reducer <= 0.5
