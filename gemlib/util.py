...
def compute_state(initial_state, events, stoichiometry, closed=False):
    ...
    :return: a tensor of shape [M, T, S] if `closed=False` or [M, T+1, S] if `closed=True`
    ...
    if closed is False:
        ...
        cum_increments = tf.concat(
            [tf.zeros_like(cum_increments[..., 0:1, :]), cum_increments], axis=-2
        )
...
def transition_coords(stoichiometry):
    src = np.where(tf.get_static_value(stoichiometry) == -1)[1]
    dest = np.where(tf.get_static_value(stoichiometry) == 1)[1]
